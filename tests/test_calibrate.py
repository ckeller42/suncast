...
def test_best_window_picks_peak_block():
    base = datetime(2026, 7, 3, 6, tzinfo=UTC)
    pts = [
        ForecastPoint(base.replace(hour=h), w)
        for h, w in [(6, 10), (7, 20), (8, 50), (9, 200), (10, 200), (11, 200), (12, 200), (13, 50)]
    ]
...
def test_best_window_partial_day_uses_available_points():
    base = datetime(2026, 7, 3, 22, tzinfo=UTC)
    pts = [ForecastPoint(base, 40.0), ForecastPoint(base.replace(hour=23), 60.0)]
    w = best_window(pts, hours=4)["2026-07-03"]
    assert w["wh"] == 100.0
    assert w["start"].startswith("2026-07-03T22")
    assert w["end"].startswith("2026-07-04T00")
