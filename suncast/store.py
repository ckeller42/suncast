...
class Store:
    ...
    def save_snapshot(
        self, s: ForecastSeries, lat: float, lon: float, panel: PanelConfig
    ) -> int:
        ...
        panel_json = json.dumps(
            {
                "panel_wp": panel.panel_wp,
                "tilt_deg": panel.tilt_deg,
                "azimuth_deg": panel.azimuth_deg,
                "charger_limit_w": panel.charger_limit_w,
                "damping": panel.damping,
            }
        )
    ...
    def has_snapshot_today(self, day: str) -> bool:
        cursor = self.conn.execute(
            "SELECT 1 FROM snapshots WHERE day = ? LIMIT 1", (day,)
        )
    ...
    def has_ratio(self, day: str) -> bool:
        cursor = self.conn.execute(
            "SELECT 1 FROM ratios WHERE day = ? LIMIT 1", (day,)
        )
        return cursor.fetchone() is not None

    def get_panel(self) -> PanelConfig:
        cursor = self.conn.execute(
            "SELECT json FROM panel WHERE id = 1"
        )
        row = cursor.fetchone()
        ...
        return PanelConfig(
            panel_wp=panel_data["panel_wp"],
            tilt_deg=panel_data["tilt_deg"],
            azimuth_deg=panel_data["azimuth_deg"],
            charger_limit_w=panel_data["charger_limit_w"],
            damping=panel_data["damping"],
        )
        ...
        panel_json = json.dumps(
            {
                ...
                "charger_limit_w": p.charger_limit_w,
                "damping": p.damping,
            }
        )
        self.conn.execute(
